...
# Dynamically determine host IP for Ollama reachability from Podman containers
HOST_IP=$(podman run --rm agent-zero-local:ready python3 -c "import socket; print(socket.gethostbyname('host.containers.internal'))" 2>/dev/null || echo "")
